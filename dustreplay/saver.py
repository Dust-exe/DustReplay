"""DustReplay v4.0 — Replay saver.

Merges MKV ring-buffer segments into a single MP4 file.
Stream-copy first (instant), re-encode as fallback.
...
def _merge_export(ff: str, lp: str, out: str, valid_segs: list) -> Tuple[bool, str]:
    """Merge MKV segments to MP4.

    Strategy: stream-copy first (instant) → re-encode if copy fails.
    v4.0: MKV segments have consistent timestamps, so stream-copy
    should succeed in almost all cases.
    """
    # ── Attempt 1: Stream copy (instant) ──
    r0 = subprocess.run(
        [
            ff, "-y",
            "-f", "concat",
    ...
    # ── Attempt 2: Re-encode with preferred encoder ──
    ...
    venc = encoding.video_encode_args(enc, cq, fps)
    ...
    cmd = [ff, "-y", "-f", "concat", "-safe", "0", "-i", lp, *venc]
    if has_audio:
    ...
    # ── Attempt 3: CPU fallback if GPU encode failed ──
    if r.returncode != 0 and enc != encoding.ENC_CPU:
        logger.warning("GPU export failed (%s), trying libx264…", enc)
        venc2 = encoding.video_encode_args(encoding.ENC_CPU, cq, fps)
        cmd2 = [ff, "-y", "-f", "concat", "-safe", "0", "-i", lp, *venc2]
        if has_audio:
        ...
        r = subprocess.run(
            cmd2, capture_output=True, timeout=900,
            creationflags=_subprocess_flags(),
        )

    # ── Attempt 4: Video-only if audio is the problem ──
    if r.returncode != 0 and has_audio:
        logger.warning("Re-encode with audio failed; retrying video-only (-an).")
        venc3 = encoding.video_encode_args(encoding.ENC_CPU, cq, fps)
        cmd3 = [ff, "-y", "-f", "concat", "-safe", "0", "-i", lp, *venc3,
                "-an", "-movflags", "+faststart", out]
        r = subprocess.run(
            cmd3, capture_output=True, timeout=900,
            creationflags=_subprocess_flags(),
        )
...
def _worker(recorder, minutes, on_done, on_error, watchdog=None):
    try:
        ...
        # v4.0: No need to pause watchdog — FFmpeg is never stopped.
        segs = recorder.get_segments_for_export(minutes)

        if not segs:
            seg_count = len(glob.glob(os.path.join(config.TEMP_DIR, "seg_*.mkv")))
            if seg_count == 0:
                if on_error:
